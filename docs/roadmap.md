...
**Current priority:** finish presentation and usage review of the verified one-model package, stage a clean private Hugging Face history, and verify its pinned download before publication. The [final selection and locked release evaluation](final-release-selection.md#completed-results) completed all three million rounds and retained the teacher-cost research incumbent. Continuous-play gains were +0.30854 units per 100 rounds over basic strategy and +0.27458 over packaged v0.2, with positive familywise intervals; fresh-shoe differences remain inconclusive and absolute returns remain negative. The [independent SDK replication](model-visited-training.md#replication-results) completed all 1.6 million rounds without establishing an improvement: model-visited selection retained the incumbent, while the control had higher final reference regret and unresolved return gains. The research incumbent remains retained. The [completed 12-million-round comparison and error analysis](large-return-results.md) found a continuous-play advantage over the basic heuristic (+0.1989 units per 100 rounds; adjusted interval +0.0862 to +0.3116), with no resolved advantage over the predecessor. Fresh-shoe comparisons remain inconclusive. The completed [teacher-cost study](teacher-cost-experiment.md) improved same-test agreement from 94.96% to 95.90% and lowered reference regret 35.54%, but its two loss objectives were nearly tied on selection.

The earlier 12-million-round study's diagnostic reference regret concentrates in the enriched depleted-shoe test stratum (69.76%) and hit-versus-stand confusions (52.82%). Fresh-world rechecks retained the reference action's advantage in 20 of 25 selected Monte Carlo cases after multiplicity correction. These findings support a bounded collection/labeling pilot followed, if qualified, by a data-mixture comparison with matched state counts and compute. They do not justify replaying inspected test errors into training or launching a longer version of the same objective sweep.
...
The [exact-reference and decision-cost comparison](teacher-cost-experiment.md) and [larger return analysis](large-return-results.md) are complete. The latest candidate remains the research incumbent based on its frozen selection and improved reference metrics; it is not a demonstrated return winner over its predecessor or a final public release selection. The first model-visited pilot missed its label-resolution gate (78% versus 80%); its fresh-seed stronger-label follow-up passed every unchanged check (87.85% model-visited, 89.00% mixture). The [matched training comparison](model-visited-training.md#completed-results) completed both training arms and all 800,000 return rounds. A preserved single-state batch-parity failure was handled through exact-SDK evaluation under the original deadline. The model-visited arm won selection, but final regret and return differences from the incumbent remain unresolved; it stays private and inactive. The independent 1.6-million-round replication also found no established improvement. Stronger split/multiplayer references and reward refinement remain possible separate studies.
