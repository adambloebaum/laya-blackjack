...
function renderReport(report) {
  ...
  $('training-report').innerHTML=`<table class="report-table"><thead><tr><th>Frozen test metric · ${release||report.test_inference?'SDK':'trainer'}</th><th>${escape(baselineLabel)}</th><th>${release?'Laya Blackjack':'Trained / calibrated'}</th></tr></thead><tbody>${rows.map(([label,key,up,percent])=>{const a=measured.baseline[key],b=measured.test[key];return `<tr><td>${label}</td><td>${percent?pct(a,1):a.toFixed(4)}</td><td class="${(up?b>a:b<a)?'improved':'worse'}">${percent?pct(b,1):b.toFixed(4)}</td></tr>`;}).join('')}</tbody></table><p class="footnote">${escape(report.limitations)}${selection}</p>`;
}
