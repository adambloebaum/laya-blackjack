...
def main():
    ...
    fig.subplots_adjust(left=0.255, right=0.70, top=0.73, bottom=0.25)
    fig.text(0.035, 0.93, "One selected model · 3 million simulated rounds", fontsize=17, weight="bold")
    ...
    for y, (mode, comparator) in zip((3, 2, 1, 0), (
        ...
        labels.append(f"{'Fresh shoe' if mode == 'fresh' else 'Continuous'} · vs "
                      f"{'basic' if comparator == 'basic' else 'packaged v0.2'}")
    ax.set_yticks([3, 2, 1, 0], labels)
    ax.tick_params(axis="y", length=0, pad=10)
    ax.set_xlim(-0.15, 0.65)
    ax.set_ylim(-0.5, 3.5)
    ax.set_xlabel("Selected-model advantage · betting units per 100 rounds", labelpad=12)
    fig.text(0.035, 0.095, "Bonferroni-adjusted 95% family coverage across four planned contrasts; normal approximation.", fontsize=9)
    fig.text(0.035, 0.045, "Per policy: 500,000 fresh rounds + 5,000 independent 100-round blocks. Absolute mean returns remained negative.", fontsize=9)
    save(fig, "final-returns")

    fig, axes = plt.subplots(1, 2, figsize=(11.8, 4.5))
    fig.subplots_adjust(left=0.16, right=0.97, top=0.71, bottom=0.25, wspace=0.65)
    fig.text(0.035, 0.93, "Closer to the reference on a fresh locked test", fontsize=17, weight="bold")
    fig.text(0.035, 0.865, "16,384 states · 4,840 game groups · canonical three-question SDK", fontsize=11)
    for ax, metric, interval, title, scale in (
        ...
        ax.set_yticks([1, 0], ["Packaged v0.2", "Selected model"])
    ...
    fig.text(0.035, 0.095, "Individual 95% whole-game bootstrap intervals, conditional on recorded reference labels.", fontsize=9)
